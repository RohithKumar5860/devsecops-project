...
# ── APP_SECRET_KEY fail-fast validation ───────────────────────────────────────
# In development: auto-generate an ephemeral key if not provided.
# In staging / production: the key MUST be injected via the environment;
# missing key is a hard startup failure to prevent insecure deployments.
_secret_key = os.environ.get("APP_SECRET_KEY")

if _secret_key is None:
    if APP_ENV == "development":
        _secret_key = _secrets.token_hex(32)   # ephemeral – never committed
    else:
        raise RuntimeError(
            "APP_SECRET_KEY environment variable must be set in non-development "
            f"environments. Current APP_ENV={APP_ENV!r}. "
            "Inject the secret via your CI/CD pipeline or external secrets manager."
        )

# ── DEBUG guard ───────────────────────────────────────────────────────────────
# Prevent accidental debug mode in production (would expose stack traces).
_debug = os.environ.get("DEBUG", "false").lower() == "true"
if _debug and APP_ENV == "production":
    raise RuntimeError(
        "DEBUG cannot be enabled in production (APP_ENV='production'). "
    ...
    return jsonify(
        {
            "metadata": {
                "name": APP_NAME,
                "description": (
                    "A production-ready DevSecOps project demonstrating modern CI/CD "
                    "automation, shift-left security practices, Docker containerization, "
                    "and Kubernetes-ready deployment"
                ),
                "version": VERSION,
                "environment": APP_ENV,
            },
            ...
            "endpoints": [
                {"path": "/", "description": "Main Project Dashboard", "method": "GET"},
                {"path": "/api/project", "description": "Aggregated project information", "method": "GET"},
                {"path": "/api/pipelines", "description": "CI/CD pipeline metadata", "method": "GET"},
                {"path": "/health", "description": "Health check endpoint", "method": "GET"},
                {"path": "/api/info", "description": "Application information", "method": "GET"},
            ],
        }
    ), 200
